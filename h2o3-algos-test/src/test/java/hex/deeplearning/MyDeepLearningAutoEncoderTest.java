...
public class MyDeepLearningAutoEncoderTest extends TestUtil {
	/*
	 * Visualize outliers with the following R code (from smalldata/anomaly dir):
	 * 
	 * train <- scan("ecg_discord_train.csv", sep=",") test <-
	...
	public void run() {
		...
		try {
			...
			try {
				...
				// Reconstruct data using the same helper functions and verify that
				// self-reported MSE agrees
				...
				for (int r = 0; r < reconstr.numRows(); ++r) {
					...
					for (int c = 0; c < reconstr.numCols(); ++c) {
						my_l2 += Math.pow((reconstr.vec(c).at(r) - train.vec(c).at(r))
								* mymodel.model_info().data_info()._normMul[c], 2); // undo normalization here
					}
				...
				sb.append("The following training points are reconstructed with an error above the ")
						.append(quantile * 100).append("-th percentile - check for \"goodness\" of training data.\n");
